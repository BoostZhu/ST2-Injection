'''
cloned from https://github.com/ragor114/PyTorch-Frechet-Video-Distance/blob/main/fvd_metric/fvd.py
'''
"""
DISCLAIMER
...
"""
def compute_feature_stats(data, detector_url, detector_kwargs, batch_size, max_items, device,local_model_path=None):
    ...
    if local_model_path:
        print(f"Bypassing download. Loading detector from local path: {local_model_path}")
        try:
            detector = torch.jit.load(local_model_path).eval().to(device)
...
"""
def compute_fvd(y_true: torch.Tensor, y_pred: torch.Tensor, max_items: int, device: torch.device, batch_size: int, local_model_path=None):
    ...
    mu_true, sigma_true = compute_feature_stats(y_true, detector_url, detector_kwargs, batch_size, max_items, device, local_model_path=local_model_path)
    mu_pred, sigma_pred = compute_feature_stats(y_pred, detector_url, detector_kwargs, batch_size, max_items, device, local_model_path=local_model_path)
