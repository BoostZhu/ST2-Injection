...
def get_activations(data, model, batch_size=10):
    ...
    for batch in batch_generator(data, batch_size):
        # The model runs on the GPU, so its output is a GPU tensor.
        # We must move it to the CPU before converting to numpy.
        batch_activations = model(batch).squeeze().detach().cpu().numpy() # <--- THE FIX IS HERE
...
def frechet_video_distance(first_set_of_videos, second_set_of_videos, path_to_model_weights,device):
    ...
    try:
        # Pytorch 1.13+
        i3d.load_state_dict(torch.load(path_to_model_weights, weights_only=True))
    except TypeError:
